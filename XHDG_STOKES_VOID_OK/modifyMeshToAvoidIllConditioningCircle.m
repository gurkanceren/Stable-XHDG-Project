...
for i=1:length(nodesCloseInt)
    ...
    if minDistance-1.e10<1e-20 %it has been updated with a face :-)
...
%STEP 2: Nodes on the 4 "corners" are moved towards the center
nodesC=[find(abs(X(:,1))<1.e-10 & (abs(abs(X(:,2))-0.5))<1.e-10)
        find(abs(X(:,2))<1.e-10 & (abs(abs(X(:,1))-0.5))<1.e-10)];
X(nodesC,:)=(1-2.5*tol)*X(nodesC,:);
