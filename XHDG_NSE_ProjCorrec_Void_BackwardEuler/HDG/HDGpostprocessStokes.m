function [u_star,N] = HDGpostprocessStokes(X,T,u,L,referenceElement_star,referenceElement,Elements,mu,example)
...
for iElem = 1:nOfElements   
    ...
    %1st component
    ue = N*u(ind,1);
    qe(1:2:2*npoints) = N*L(ind,1);
    qe(2:2:2*npoints) = N*L(ind,2);
    [Ke,Beqe,int_ue_star, int_ue] = ElementalMatrices(X(T(iElem,:),:),Xe_star,referenceElement_star,ue,qe,Elements,LSe_star,iElem);
    K = [Ke  int_ue_star; int_ue_star' 0]; % Lagrange multipliers
    f = [Beqe;int_ue];
    sol = K\f;
    u_star(ind_star,1) = sol(1:end-1);
    %2nd component
    ue = N*u(ind,2);
    qe(1:2:2*npoints) = N*L(ind,3);
    qe(2:2:2*npoints) = N*L(ind,4);
    [Ke,Beqe,int_ue_star, int_ue] = ElementalMatrices(X(T(iElem,:),:),Xe_star,referenceElement_star,ue,qe,Elements,LSe_star,iElem);
    K = [Ke int_ue_star; int_ue_star' 0]; % Lagrange multipliers
    f = [Beqe;int_ue];
    sol = K\f;
...
function [K,Bq,int_u_star,int_u] = ElementalMatrices(Xe,Xe_star,referenceElement_star,ue,qe,Elements,LSe_star,iElem)
