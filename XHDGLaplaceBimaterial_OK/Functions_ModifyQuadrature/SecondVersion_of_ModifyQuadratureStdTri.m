...
elseif LSe_Vertices <= 0
    ...
    if length(Vert0) == 1
        ...
        %Pts = locatePointsOnCircle(referenceElement.NodesCoord1d,Pts,Xe,[0,0]);
...
function newPoints = locatePointsOnCircle(coord1D,Points,Xe,circumferenceCenter)
...
%center with local coordinates
C = inverseIsoparametricLinearTriangle(Xe,circumferenceCenter);

%radius
r = norm(P1-C);

theta1 = atan(P1(2)/P1(1));
theta2 = atan(P2(2)/P2(1));

if theta2 > theta1
    theta1 = theta1 + 2*pi;
end

theta = theta1 + (theta2-theta1)*(coord1D+1)/2;

newPoints = repmat(C,p+1,1) + r*[cos(theta) sin(theta)];



function xieta = inverseIsoparametricLinearTriangle(Xe,x)
% function xieta = inverseIsoparametricLinearTriangle(Xe,x)
% Xe: vertex nodes
% x: phisical coordinates
% xieta: reference element coordinates
