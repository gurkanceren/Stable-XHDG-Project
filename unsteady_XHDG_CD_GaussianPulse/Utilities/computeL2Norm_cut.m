function L2Norm = computeL2Norm_cut(LSe,referenceElement,X,T,u,u0,time,varargin)
...
for iElem = 1:nOfElements
    ...
    Xe = X(Te,:);
    ue = u(Te);
    L2Norm = L2Norm + ElementalL2Norm(Xe,ue,u0,N,Nxi,Neta,weight,ngauss,time,varargin{:});
end
L2Norm = sqrt(L2Norm);


%_______________________________________________________________________
function elemL2Norm = ElementalL2Norm(Xe,ue,u0,N,Nxi,Neta,weight,ngauss,time,varargin)
...
for g = 1:ngauss
    ...
    u0_g = analiticalSolutionLaplace(xy_g,time);
    %Jacobian
    J = [Nxi_g*xe	  Nxi_g*ye
        Neta_g*xe  Neta_g*ye];
    %Integration weight
