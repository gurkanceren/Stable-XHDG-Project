function [KK,f, QQ, UU, Qf, Uf] = hdg_matrix(X,T,F,referenceElement,infoFaces,tau,LS,Elements,mu1,mu2)
...
for iElem = 1:nOfElements
    ...
    % elemental matrices
    [Qe,Ue,Qfe,Ufe,Alq,Alu,All] = KKeElementalMatrices(Xe,referenceElement,tau,LSe,iElem,Elements,mu1,mu2);
...
% ELEMENTAL MATRIX
function [Q,U,Qf,Uf,Alq,Alu,All] = KKeElementalMatrices(Xe,referenceElement,tau,LSe,iElem,Elements,mu1,mu2)
...
sourceTerm = analiticalLaplacianLaplace(Xg,mu1);
...
%     quiver(I(:,1),I(:,2),Iprime(:,1),Iprime(:,2))
    ...
    for igauss=1:g
        ...
        AuhatuhatI= AuhatuhatI-tau_f*referenceElement.N1d(igauss,:)'*referenceElement.N1d(igauss,:)*wg;
        ge= ge+referenceElement.N1d(igauss,:)'*-NeumannCondition(n_g,mu1)*wg;
    ...
    Auqnew=Auq+AuuhatI*Bq;
    Auunew=Auu+AuuI+AuuhatI*Bu;
    Aqunew=Aqu+AquhatI*Bu;
    Aqqnew=Aqq+AquhatI*Bq;
    ...
    UQ = -A\[Aul;Aql];
    fUQ= A\[(fe-(AuuhatI*be));-(AquhatI*be)];
...
% %     q_analy=-mu*ones((2*size(u_analy,1)),1);
% %     q_analy(2:2:end)=0;
% %     
% %     
...
% %     
% %     
% %     test_result_1=fe-(Auunew*u_analy)-(Auqnew*q_analy)-(Aul*gama)-(AuuhatI*be) ;
% %     test_result_2= (Aqqnew*q_analy)+(Aqunew*u_analy)+(Aql*gama)+(AquhatI*be);
% %     
...
% %     test=AuhatqI*q_analy+AuhatuI*u_analy+AuhatuhatI*uhat_analy-ge;
% %     
...
% %     
% %     
% %     test3=Bq*q_analy+Bu*u_analy+be-uhat_analy;
% %     
