function [KK,f, QQ, UU, Qf, Uf] = hdg_matrix_predictorStepDirichlet(X,T,F,referenceElement,infoFaces,tau,LS,Elements,dt,cnt,u_old,u_old2,u_old3,p_old,time,Re,a_parm)
...
for iElem = 1:nOfElements
    ...
    %
    % elemental matrices
    [Qe,Ue,Qfe,Ufe,Alq,Alu,All,gn_press] = KKeElementalMatrices(Xe,referenceElement,LSe,Elements,tau,dt,cnt,u_Elem_old,u_Elem_old2,u_Elem_old3,p_Elem,time,Re,a_parm,iElem);
    %{
...
% ELEMENTAL MATRIX
function [Q,U,Qf,Uf,Alq,Alu,All,gn_press] = KKeElementalMatrices(Xe,referenceElement,LSe,Elements,tau,dt,cnt,u_Elem_old,u_Elem_old2,u_Elem_old3,p_Elem,time,Re,a_parm,iElem)
...
if  isempty(c) && isempty (d) % for interface I of the cut element
    ...
    %press_I_e=analyticalpressure(Pgauss,time,Re);
    %
    if(cnt==1)
