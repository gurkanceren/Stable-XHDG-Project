function vel=analyticalVelocityStokes(X,time,Re)
...
par = 2*pi;
%
ux = pi.*sin(time).*sin(par.*y).*(sin(pi.*x)).^2;
uy = -pi.*sin(time).*sin(par.*x).*(sin(pi.*y)).^2;
%
vel = [ux ; uy]; 
%}
%
...
%
u_x = -1.*cos(a).*sin(b)*exp(pow);
%
u_y = sin(a).*cos(b)*exp(pow);
%
vel = [u_x ; u_y];
%}
%
