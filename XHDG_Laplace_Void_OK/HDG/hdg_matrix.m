...
sourceTerm = analiticalLaplacianLaplace(Xg,mu);
...
for g = 1:ngauss
    ...
    Aqq = Aqq + NN_g'*NN_g*dvolu*1/mu;       %%%%%%%%%%%% IMPORTANNTTTT  %%%%%%%%%%%%%%%
...
if  isempty(c) && isempty (d)
    ...
    for igauss=1:g
        ...
        AuuhatI= AuuhatI-Nzg(igauss,:)'*referenceElement.N1d(igauss,:)*wg;
        AuuI= AuuI+Nzg(igauss,:)'*Nzg(igauss,:)*wg;
        AquhatI_x_f =  AquhatI_x_f+tau_f*Nzg(igauss,:)'*referenceElement.N1d(igauss,:)*n_g(1)*wg;
        AquhatI_y_f =  AquhatI_y_f+tau_f*Nzg(igauss,:)'*referenceElement.N1d(igauss,:)*n_g(2)*wg;
...
%     test_result_2= (Aqq*q_analy)+(Aqu*mu*u_analy)+(Aql*gama)+(AquhatI*uhatI_e);
%     test_result_2= (Aqq*q_analy)+(Aql*gama)+(AquhatI*uhatI_e);
%     test_result_3=  (Aqu*u_analy*-mu);    
% 
...
%         disp(['Test result equation 3 for element ' num2str(iElem) ]);
%     disp(max(abs(test_result_3)));
...
%     test_result_2= (Aqq*q_analy)+(Aqu*mu*u_analy)+(Aql*gama);
%     test_result_2= (Aqq*q_analy)+(Aql*gama);
%     test_result_3=  (Aqu*u_analy*-mu);    
