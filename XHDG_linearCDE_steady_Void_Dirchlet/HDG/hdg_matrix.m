...
function [KK,f, QQ, UU, Qf, Uf] = hdg_matrix(X,T,F,referenceElement,infoFaces,tau,LS,Elements,mu,c_x,c_y,upwind)
...
for iElem = 1:nOfElements
    ...
    % elemental matrices
    [Qe,Ue,Qfe,Ufe,Alq,Alu,All] = KKeElementalMatrices(Xe,referenceElement,tau,LSe,iElem,Elements,mu,c_x,c_y,upwind);
...
% ELEMENTAL MATRIX
function [Q,U,Qf,Uf,Alq,Alu,All] = KKeElementalMatrices(Xe,referenceElement,tau,LSe,iElem,Elements,mu,c_x,c_y,upwind)
...
if  isempty(c) && isempty (d)
    ...
    uhatI_e_old=analiticalSolutionLaplace(Pphy);
...
% %     
% %     
     test_result_1=fe-(Auu*u_analy)-(Auq*q_analy)-(Aul*gama)+(AuuhatI*uhat_analy) ;
