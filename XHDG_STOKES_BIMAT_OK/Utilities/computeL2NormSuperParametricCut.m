function L2Norm = computeL2NormSuperParametricCut(LSe,referenceElement,X,T,u,u0,varargin)
...
for iElem = 1:nOfElements 
    Te = T(iElem,:); 
    Xe = X(Te,:);
    %ue = u(2*nOfElementNodes*(iElem-1)+[1:2*nOfElementNodes]);  %not necessary
    L2Norm = L2Norm + ElementalL2Norm(Xe,N,Nxi,Neta,Nold,Nxi_old,Neta_old,weight,ngauss,u,u0,varargin{:}); 
end
L2Norm = sqrt(L2Norm);


%_______________________________________________________________________
function elemL2Norm = ElementalL2Norm(Xe,N,Nxi,Neta,Nold,Nxi_old,Neta_old,weight,ngauss,ue,u0,varargin)
...
for g = 1:ngauss
    ...
    N_g = N(g,:);
    Nxiold_g = Nxi_old(g,:);
    Netaold_g = Neta_old(g,:);
    xy_g = Nold(g,:)*Xe;
    ue_g = N_g*ue;
    u0_g = u0(xy_g,varargin{:});
    %Jacobian
    J = [Nxiold_g*xe	  Nxiold_g*ye   
         Netaold_g*xe  Netaold_g*ye];
    %Integration weight
