%function [u_star,shapeFunctions] = hdg_postprocess(X,T,u,L,referenceElement_star,referenceElement,mu_vector)
function [u_star,shapeFunctions] = hdg_postprocess(X,T,u,L,referenceElement_star,referenceElement,Re)
...
u_star = zeros(npoints*nOfElements,2);
...
for iElem = 1:nOfElements
    ...
    %first component
    ue = shapeFunctions*u(ind,1);
    %qe(1:2:2*npoints) = shapeFunctions*q(2*ind-1);
    %qe(2:2:2*npoints) = shapeFunctions*q(2*ind);
    qe(1:2:2*npoints) = shapeFunctions*L(ind,1);
    qe(2:2:2*npoints) = shapeFunctions*L(ind,2);
    %mu=mu_vector(iElem);
    [Ke,Beqe,int_ue_star, int_ue] = ElementalMatrices(Xe,referenceElement_star,ue,qe,Re);
    ...
    u_star(ind_star,1) = sol(1:end-1);
    %second component
    ue = shapeFunctions*u(ind,2);
    %qe(1:2:2*npoints) = shapeFunctions*q(2*ind-1);
    %qe(2:2:2*npoints) = shapeFunctions*q(2*ind);
    qe(1:2:2*npoints) = shapeFunctions*L(ind,3);
    qe(2:2:2*npoints) = shapeFunctions*L(ind,4);
    %mu=mu_vector(iElem);
    [Ke,Beqe,int_ue_star, int_ue] = ElementalMatrices(Xe,referenceElement_star,ue,qe,Re);
    ...
    % postprocessed solution
    u_star(ind_star,2) = sol(1:end-1);
end

%%
%% ELEMENTAL MATRIX

function [K,Bq,int_u_star,int_u] = ElementalMatrices(Xe,referenceElement_star,ue,qe,Re)
...
for g = 1:ngauss
    ...
    Bq = Bq + (Nx_g'*qx_g + Ny_g'*qy_g)*dvolu; 
    K = K + (Nx_g'*Nx_g + Ny_g'*Ny_g)*dvolu/Re;
