function [KK,f, QQ, UU, Qf, Uf, uDirichlet] = hdg_matrix(X,T,F,referenceElement,infoFaces,tau,mu_vector)
...
for iElem =1:nOfElements
    ...
    % elemental matrices
    [Qe,Ue,Qfe,Ufe,Alq,Alu,All] = KKeElementalMatrices(Xe,referenceElement,tau,mu);
...
%%
%% ELEMENTAL MATRIX
function [Q,U,Qf,Uf,Alq,Alu,All] = KKeElementalMatrices(Xe,referenceElement,tau,mu)
...
for g = 1:ngauss
    ...
    Auu_vol_x = -Nx_g'*Nx_g*dvolu;
    Auu_vol_y = -Ny_g'*Ny_g*dvolu;
    Auu_vol = Auu_vol + (Auu_vol_x+Auu_vol_y);
    % fe = fe - N_g'*sourceTerm(xy_g)*mu*dvolu;
    fe = fe + N_g'*sourceTerm(xy_g)*dvolu;
end
%inv_Aqq = inv(Aqq);
