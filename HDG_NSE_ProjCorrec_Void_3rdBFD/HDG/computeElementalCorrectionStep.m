function [u_crc, p_crc]=computeElementalCorrectionStep(u,uhat,pcorr,phat,qcorr,X,T,F,referenceElement,infoFaces,tauP,dt,Re,a_parm,time,cnt,pold)
...
for iElem =1:nOfElements
    ...
    [u_lcl, p_lcl]=ElementalVariables(Fe,Xe,referenceElement,nOfInteriorFaces,infoFaces,tauP,dt,Re,a_parm,uE,pcorrE,qcorrE,uhatE,phatE,time,cnt,poldE,iElem);
    %
...
function [u_lcl, p_lcl] = ElementalVariables(Fe,Xe,referenceElement,nOfInteriorFaces,infoFaces,tau,dt,Re,a_parm,uE,pcorrE,qcorrE,uhatE,phatE,time,cnt,poldE,iElem)
...
fe_uPred_vol = zeros(nOfElementNodes,1);
fe_uPred_surf = zeros(nOfElementNodes,1);
...
for g = 1:ngauss
    ...
    N_gradT = N_grad';
    %
    fe_uPred_vol = fe_uPred_vol + N_gradT*uPred_gauss*dvolu/Re;
...
for iface = 1:nOfFaces
    ...
    fe_uc_f = zeros(nOfFaceNodes,1);
    fe_up_f = zeros(nOfFaceNodes,1);
    ...
    for g = 1:ngauss_f
        ...
        %
        up_gaussFace = Nf_g*uE(nodes,:);
        %
        upg_n = n_g.*up_gaussFace;
        %
        dd = Nf_g'*(upg_n(1) + upg_n(2))*dline/Re;
        %
        fe_up_f = fe_up_f - dd;        
        % 
    ...
    fe_uPred_surf(nodes) = fe_uPred_surf(nodes) + fe_up_f;
    % 
...
% Elemental mapping
fp = fe_press + (fe_uPred_vol + fe_uPred_surf) + fe_uCorrect;
%
%p_1 = inv(App)*fp;
%
p_intm = App\fp;

cnt_intm = size(p_intm);
%
for i=1:cnt_intm
    %
    cg = p_intm(i);
